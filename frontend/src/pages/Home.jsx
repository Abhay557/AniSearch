...
export function AnimeCard({ anime, index }) {
  ...
  const safeId = encodeURIComponent(anime.title);
  const routeId = anime.mal_id ? anime.mal_id : safeId;

  return (
    <Link
      to={`/anime/${routeId}`}
      state={{ anime }}
      className="anime-card block group aspect-[2/3] stagger-item"
    ...
    >
      ...
      <div className="relative z-10 p-5 flex flex-col justify-end h-full">
        ...
        <div className="flex items-center gap-2 mb-3 text-[12px] text-zinc-500 font-medium whitespace-nowrap">
          <span className="flex items-center gap-1">
            <Star className="w-3 h-3 text-amber-400" fill="currentColor" />
            {anime.mal_score || 'N/A'}
          </span>
          <span className="flex-shrink-0 w-1 h-1 rounded-full bg-zinc-700" />
          <span className="truncate">{anime.episodes === '?' || anime.episodes == null ? 'Ongoing' : `${anime.episodes} eps`}</span>
        </div>
...
/* ─── Main App ────────────────────────────── */
export default function Home({ cachedResults, setCachedResults, cachedQuery, setCachedQuery, hasSearched, setHasSearched }) {
  ...
  return (
    <main className="relative z-10 max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {/* ─── Empty State ─── */}
      {!isSearching && hasSearched && cachedResults.length === 0 && !error && (
